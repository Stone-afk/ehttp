		...
		trees: map[string]*node{
			http.MethodGet: {
				...
				children: map[string]*node{
					...
					"sku": {path: "sku", regChild: &node{
						path: ":id(^[0-9]+$)", handler: mockHandler}},
					"profile": {path: "profile", regChild: &node{
						path: ":username(^[a-z]+$)", handler: mockHandler}},
				},
				starChild: &node{
					path: "*",
					children: map[string]*node{
	...
	testCases := []struct {
		...
		},
		{
			...
			mi: &matchInfo{
				...
				pathParams: map[string]string{"username": "stone"},
			},
		},
	}
	r := newRouter()
...
func Test_findRoute_Middleware(t *testing.T) {
	...
	mdlsRoute := []struct {
		...
		},
		{
			method: http.MethodGet,
			path:   "/a/:id(^[0-9]+$)/c",
			mdls:   []Middleware{mdlBuilder('a'), mdlBuilder('r'), mdlBuilder('c')},
		},
		{
			method: http.MethodPost,
			path:   "/a/b/*",
